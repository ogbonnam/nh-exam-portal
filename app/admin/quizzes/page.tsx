...
export default async function AdminQuizzesPage(props: any): Promise<React.ReactElement> {
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex gap-4 mb-6">
        <form method="GET" className="flex gap-2 items-center flex-wrap">
          ...
          <select name="className" defaultValue={classNameFilter || ""}>
            <option value="">All</option>
            {(yearGroupFilter ? yearGroupClasses[yearGroupFilter] || [] : []).map((c) => (
          ...
          {quizzes.map((quiz) => (
            ...
            >
              <div>
                ...
                <p
                  className={`mt-2 text-sm font-semibold ${quiz.isPublished ? "text-green-600" : "text-red-600"}`}
                >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <LockQuizButton quizId={quiz.id} canTeacherEdit={quiz.canTeacherEdit} />
      ...
      <div className="flex justify-center mt-6 gap-2">
        {Array.from({ length: totalPages }).map((_, i) => (
          <Link
            key={i}
            href={`?page=${i + 1}&teacherId=${teacherFilter || ""}&yearGroup=${yearGroupFilter || ""}&className=${classNameFilter || ""}`}
