...
//   return (
//     <div className="min-h-screen bg-gray-50 flex flex-col my-10">
//       {/* Header */}
...
export default function QuizClient(props: any) {
  ...
  // audio & alert configuration
  const ALERT_THRESHOLD_SEC = 300; // warn at 5 minutes (300s) left
  const FINAL_BEEP_AT_SEC = 5; // final beep in last 5s
  const warningPlayedRef = useRef(false);
  const finalBeepPlayedRef = useRef(false);
  const audioUnlockedRef = useRef(false);
  const focusWarningPlayedRef = useRef(false);
  ...
  const speak = useCallback((text: string, urgent = false) => {
    ...
    utterance.rate = 1;
    utterance.pitch = 1;
    utterance.volume = urgent ? 1 : 0.8; // Louder for urgent messages
  ...
  // Handle visibility change for focus loss warning
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden && !isSubmitted) {
        const now = Date.now();
        // Only play warning if it's been at least 30 seconds since the last one
        if (now - lastFocusWarningTimeRef.current > 30000) {
          lastFocusWarningTimeRef.current = now;
          speak("You have gone off the exam page. Please return immediately.", true);
          setShowWarning(true);
  ...
  // submit request (used by manual and auto)
  async function doSubmitOnce(submissionReason: 'user' | 'system' = 'user') {
    try {
      // ensure latest answers are persisted via the normal save (still awaited)
      await handleSave();
      ...
      const payloadBody = { 
        attemptId, 
        answers: formatForServer(),
        submissionReason // Add submission reason
      };
  ...
  const autoSubmitAndRedirect = useCallback(async () => {
    ...
    // 1) Try normal submit (await it)
    const r = await doSubmitOnce('system');
    if (r.ok) {
      try { localStorage.removeItem(LS_KEY); } catch {}
      setIsSubmitted(true);
      console.log("Auto-submit success (normal fetch)", r.data);
      // small delay so server logs/DB update can complete
      setTimeout(() => router.push("/student/dashboard?submitted=true&submittedBy=system"), 300);
      return;
    }
    ...
    try {
      if (typeof navigator !== "undefined" && typeof navigator.sendBeacon === "function") {
        ...
        setTimeout(() => {
          try { localStorage.removeItem(LS_KEY); } catch {}
          setIsSubmitted(true);
          router.push("/student/dashboard?submitted=true&submittedBy=system");
        }, 1000);
        return;
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    let interval = window.setInterval(() => {
      ...
      // play warning once when 5 minutes threshold crossed
      if (!warningPlayedRef.current && secs <= ALERT_THRESHOLD_SEC && secs > FINAL_BEEP_AT_SEC) {
        warningPlayedRef.current = true;
        playBeep(880, 300, "sine");
        speak("Few minutes to the end of the exam.");
      }
    ...
    return () => clearInterval(interval);
  }, [computeMsLeft, endTimeISO, serverNowISO, isSubmitted, autoSubmitAndRedirect, speak]);
  ...
  const handleSubmit = async () => {
    ...
    if (r?.ok) {
      try { localStorage.removeItem(LS_KEY); } catch {}
      setIsSubmitted(true);
      router.push("/student/dashboard?submitted=true&submittedBy=user");
    } else {
      alert("Submit failed — please try again.");
    }
  };
  ...
  async function doManualSubmit() {
    setSaveStatus("Submitting...");
    const r = await doSubmitOnce('user');
    return r;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col my-10">
      ...
      {/* Warning Banner */}
      {showWarning && (
        <div className="bg-red-50 border-l-4 border-red-400 p-4 max-w-7xl mx-auto mt-4 w-full animate-pulse">
          <div className="flex">
            <div className="flex-shrink-0">
              <AlertCircle className="h-5 w-5 text-red-400" />
            </div>
            <div className="ml-3">
              <p className="text-sm text-red-700 font-medium">
                Warning: You have left the exam page. Please return immediately.
              </p>
            </div>
          </div>
        </div>
